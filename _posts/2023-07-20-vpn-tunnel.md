...
The reason for this is because our tunnel is only sends packets in one direction (from `Host U` to `Host V`). To recieve a response to requests sent from `Host U`, we need to set up our tunnel so it also sends back sends packets in the reverse direction (from `Host V` to `Host U`).

Thus our client and server programs should be modified to support such.
...
telnet commands. In your proof, you need to point out how your packet
