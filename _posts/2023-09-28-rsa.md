...
```bash
$ python3 -c 'print(bytes.hex("I oI you $2000.".encode("ascii")))'
```
...
I Int on to copy the certificates and save them in files named c0.pem and c1.pem respectively.
...
The message hash is 64 bytes long. So to check if the certificate is valid, I will write a Python program that will compare the server's certificate body against the last 64 bytes of the computed message.
